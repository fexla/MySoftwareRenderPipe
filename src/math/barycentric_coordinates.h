...
inline std::array<float, 3> barycentric_coordinates2d(Vector2f pos, std::array<Vector2f, 3> triangle) {
    float beta, gamma;
...
inline void interpolation_correction(std::array<float, 3> &coord, const std::array<float, 3> &dep, float trueDep) {
...
interpolation(const std::array<DataType, 3> &value, const std::array<float, 3> &coord) {
